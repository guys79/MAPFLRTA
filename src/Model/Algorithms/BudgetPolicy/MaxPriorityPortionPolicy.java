...
public class MaxPriorityPortionPolicy implements IBudgetPolicy{//Type 10
    ...
    public Map<Agent, Integer> getBudgetMap(Problem problem) {
        ...
        int dev = problem.getNumberOfNodeToDevelop();
        double totalBudget = dev * agents.size();
        ...
        for(Agent agent: agents)
        {
            ...
            budget = dev/2.0 +(agent.getPriority()/sumOfPriorities)*(totalBudget/2.0);
    ...
    private void print(Map<Agent,Integer>budgets)
    {

        for(Map.Entry<Agent,Integer> e:budgets.entrySet())
        {
            System.out.println("Agent "+e.getKey().getId()+" - "+e.getValue());
        }
