...
public class BudgetOrientedMALRTA extends MAALSSLRTA {
    ...
    private Agent other;
    private Set<AlssLrtaSearchNode> leaf;
    ...
    private int aStar(int develop,MaAlssLrtaSearchNode current,int used)
    {
        ...
        while(expansions<develop-used)
        {
            ...
            if(state.getNumInChain()>prefixLength)
                break;
    ...
    protected void aStarPrecedure() {
      ...
      /*  int iter = 10;
        ...
        for(AlssLrtaSearchNode node : leaf)
        {
            openAdd(node);
        }
...
*/
    ...
    //    if(agent.getUsedBudget() == 144)
       ...
       // System.out.println("agent "+agent.getId()+" used "+agent.getUsedBudget());
        usedBudget+=agent.getUsedBudget();
